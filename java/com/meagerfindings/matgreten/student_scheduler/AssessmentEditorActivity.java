...
import static com.meagerfindings.matgreten.student_scheduler.R.array.type_array;
...
public class AssessmentEditorActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        intent = getIntent();
        uri = intent.getParcelableExtra(ScheduleContract.AssessmentEntry.CONTENT_ITEM_TYPE);
...

        {
            ...
            cursor = getContentResolver().query(uri, ScheduleContract.AssessmentEntry.ALL_ASSESSMENT_COLUMNS, assessmentFilter, null, null);

            assert cursor != null;
            cursor.moveToFirst();

            oldTitle = cursor.getString(cursor.getColumnIndex(ScheduleContract.AssessmentEntry.ASSESSMENT_TITLE));
            ...
            oldType = cursor.getString(cursor.getColumnIndex(ScheduleContract.AssessmentEntry.ASSESSMENT_TYPE));
            assessmentID = cursor.getString(cursor.getColumnIndex(ScheduleContract.AssessmentEntry.ASSESSMENT_ID));
            ...
            queryString = "SELECT * FROM " + ScheduleContract.AssessmentNoteEntry.TABLE_NAME + " WHERE " +
    ...
    private ArrayList<String> getAssessmentAlertTitles(String assessmentKeyID) {
        ...
        String queryString = "SELECT " + ScheduleContract.AssessmentAlertEntry.ASSESSMENT_ALERT_TITLE +
                " FROM " + ScheduleContract.TABLE_ASSESSMENT_ALERTS +
                " WHERE " + ScheduleContract.AssessmentAlertEntry.ASSESSMENT_ALERT_ASSESSMENT_ID_FK + " = " + assessmentKeyID;
...
//        System.out.println("Course note tiltes =" + assessmentAlertTitles);
    ...
    private void updateAssessment(String assessmentTitle, String assessmentType, String assessmentTargetEndDate, int courseID) {
        ...
        values.put(ScheduleContract.AssessmentEntry.ASSESSMENT_TYPE, assessmentType);
    ...
    public void openAssessmentAlertsList(View view) {
        switch (action) {
            ...
            case Intent.ACTION_EDIT:
                ...
                Uri uri = Uri.parse(ScheduleContract.AssessmentAlertEntry.CONTENT_URI + "/" + id);
                alertIntent.putExtra(ScheduleContract.AssessmentEntry.CONTENT_ITEM_TYPE, uri);
